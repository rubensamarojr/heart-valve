...
Real c_f = 30.0 * U_f;						//< Speed of sound.
...
Real Youngs_modulus_left = 1.3e7;				//< Young modulus.
Real Youngs_modulus_right = 1.3e4;				//< Young modulus.
...
std::vector<Vecd> createWaterBlockShape()
{
	...
	water_block_shape.push_back(Vecd(DL, -0.5 * DH + shiftCorr));
	water_block_shape.push_back(Vecd(-DL_sponge, -0.5 * DH + shiftCorr));
...
std::vector<Vecd> createTopInnerWallShape()
{
	...
	inner_wall_shape.push_back(Vecd(-DL_sponge - BW,  0.5 * DH));
	inner_wall_shape.push_back(Vecd(DL + BW,  0.5 * DH));
	inner_wall_shape.push_back(Vecd(DL + BW, -0.5 * DH - BW));
	inner_wall_shape.push_back(Vecd(-DL_sponge - BW, -0.5 * DH - BW));
...
public:
	...
	{
		// Geometry definition.
		std::vector<Vecd> outer_wall_shape1 = createOuterWallShape();
...
public:
	...
	void setupDynamics(Real dt = 0.0) override
	{
		...
		if(run_time <= 0.20)
		{
			u_ref_ = (-9291.6667 * run_time * run_time + 1893.3333 * run_time + 40.0); 
		}
		else if(run_time <= 0.25)
		{
			u_ref_ = (120.0 * (run_time - 0.2) + 47.0); 
		}
		else if(run_time <= 0.30)
		{
			u_ref_ = (-220.0 * (run_time - 0.25) + 53.0); 
		}
		else if(run_time <= 0.37)
		{
			u_ref_ = (114.2857142 * (run_time - 0.3) + 42.0); 
		}
		else
		{
			u_ref_ = (50.0 * exp(-0.4210255685 * (run_time - 0.37))); 
		}
...
int main(int ac, char *av[])
{
	...
	///SolidBodyRelationContact top_wall_contact(inserted_body_top, {&wall_boundary});
	///SolidBodyRelationContact bottom_wall_contact(inserted_body_bottom, {&wall_boundary});
	...
	MultiPolygonShape leaflet_base_shape_top(createTopLeafletBaseShape());
	MultiPolygonShape leaflet_base_shape_bottom(createBottomLeafletBaseShape());
